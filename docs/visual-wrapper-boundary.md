...
is a companion surface for the same workflow engine.
...
Enduring concept translations for Apex are:

| Reference concept | Apex-owned translation |
|-------------------|------------------------|
| Multi-pass terminal rendering | Event-composed QML scene graph with explicit state adapter, effect surface, and presentation shell. |
| Shader variants | Original Apex shader modules or QML effects selected by validated capability flags and quality tiers. |
| Burn-in / persistence | Workflow-aware phosphor trail reacting to new event rows, iterations, errors, stops, and completion. |
| Bloom / glow | Semantic glow around active provider, current spec, current iteration, warning state, and completed gates. |
| Scanlines and rasterization modes | Apex rendering modes: `modern-crisp`, `scanline`, `pixel-grid`, `subpixel`, and `cinematic`. |
| Static noise, jitter, sync, RGB shift | Independently implemented, bounded effect modules disabled by reduced-effects and plain fallbacks. |
| Profile system | Versioned XDG JSON visual profiles with import/export, reset, validation, atomic writes, and corruption backup. |
| Effects FPS and texture quality | Quality tiers: `cinematic`, `balanced`, `battery`, `low-effects`, and `plain`. |
| Desktop metadata | Original Apex icon, `.desktop`, AppStream metadata, launcher actions, AppImage, notices, inventory, and checksum. |
...
The forbidden categories include generated `.qsb` blobs, shader source,
reference image data, bundled fonts, profile JSON, QML resource manifests,
package metadata, launcher metadata, and terminal-widget submodules.
...
Before binary release, the release session must verify:
...
## Hyperterminal Design Lane Boundary

The Hyperterminal surface keeps the same event boundary while expanding the
wrapper into a production command center:

- QML consumes typed properties from the bridge; raw JSONL parsing lives in
  `visual_state.py` / `events.py` and is tested without PySide6.
- New wrapper facts arrive only through registered events
  (`spec_system_detected`, `autonomy_policy_resolved`, `task_progress`,
  `doctor_*`, `wrapper_capabilities_resolved`, and related names) or
  explicit wrapper controls; nothing is scraped from human output.
- Visual profiles persist under XDG paths
  (`~/.config/apex-infinite/visual-profiles.json`,
  `~/.local/state/apex-infinite/visual-state.json`) and never write provider
  secrets or mutate shared CLI config without explicit confirmation.
- Shader sources are Apex-owned (`shaders/PROVENANCE.md`); compiled `.qsb`
  artifacts are untracked build outputs regenerated by
  `scripts/build-shaders.sh` and reviewed before release.
- Desktop metadata, the icon, and AppImage packaging are original works
  under `src/apex_infinite_visual/assets/` and `scripts/build-appimage.sh`
  with notices in `packaging/NOTICES.md`.
- The selected scene is an event-composed command surface, not a general
  terminal-emulator viewport. `pywebview` plus `xterm.js` remains a backup only
  for a future accepted requirement that needs PTY or raw terminal fidelity.
