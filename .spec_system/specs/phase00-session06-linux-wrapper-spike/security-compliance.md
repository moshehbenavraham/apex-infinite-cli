...
- `src/apex_infinite_visual/__init__.py` - optional wrapper package exports.
- `src/apex_infinite_visual/events.py` - JSONL event parsing and state adapter.
- `src/apex_infinite_visual/launcher.py` - guarded CLI subprocess launcher.
- `src/apex_infinite_visual/main.py` - PySide6 entrypoint and QML bridge.
- `src/apex_infinite_visual/qml/Main.qml` - optional wrapper prototype surface.
- `tests/test_visual_wrapper_spike.py` - focused wrapper tests.
- `docs/visual-wrapper-spike.md` - spike evidence and decision record.
- `README.md` - optional wrapper run notes.
- `docs/visual-wrapper-boundary.md` - wrapper boundary decisions.
- `docs/operator-runbook.md` - operator wrapper notes.
- `docs/troubleshooting.md` - wrapper failure modes.
...
**Review evidence**:
- Command/check: `rg -n "(sk-[A-Za-z0-9]{20,}|Bearer [A-Za-z0-9._-]{12,}|api[_-]?key\\s*=|password\\s*=|secret\\s*=|token\\s*=|shell=True|eval\\(|exec\\(|subprocess|sqlite|INSERT|UPDATE|DELETE|SELECT|http://|https://)" src/apex_infinite_visual tests/test_visual_wrapper_spike.py docs/visual-wrapper-spike.md README.md docs/visual-wrapper-boundary.md docs/operator-runbook.md docs/troubleshooting.md`
...
- Command/check: `rg -n "cool-retro-term|TerminalWindow|ShaderTerminal|BurnInEffect|resources\\.qrc|qmltermwidget|QTermWidget|EXAMPLE/|copied terminal|GPL" src/apex_infinite_visual || true`
