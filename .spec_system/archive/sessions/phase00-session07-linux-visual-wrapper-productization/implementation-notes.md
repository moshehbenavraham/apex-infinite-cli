...
- Command/check: `rg -n 'PySide6|Nuitka|Qt Quick|AppImage' requirements.txt`
  - Result: PASS - base requirements boundary remains clean.
  - Evidence: command printed `base requirements boundary clean`.
- Command/check: `rg -n 'visual-wrapper-productization|Visual wrapper productization' README.md docs/operator-runbook.md docs/visual-wrapper-boundary.md docs/troubleshooting.md`
...
**Verification**:
- Command/check: `python -m pytest tests/ -v && cd .. && git diff --check`
...
**Files Changed**:
- `src/apex_infinite_visual/main.py` - added compatibility fallback for process wait doubles.
- `tests/test_visual_wrapper_spike.py` - updated spike assertions to productized `Malformed event` and `Exit code 1` labels.
...
**Files Changed**:
- `tests/test_visual_wrapper_productization.py` - added launcher, event adapter, and bridge behavior tests.

**Verification**:
- Command/check: `python -m pytest tests/test_visual_wrapper_productization.py -v`
...
**Files Changed**:
- `tests/test_visual_wrapper_productization.py` - added settings, parser, missing dependency, and import-isolation tests.
...
- UI product-surface check: N/A - tests only.
- UI craft check: N/A - tests only.
- BQC Fixes: Contract alignment - settings validation and import isolation are covered by focused tests (`tests/test_visual_wrapper_productization.py`).
...
**Files Changed**:
- `docs/troubleshooting.md` - added productized wrapper failure and packaging recovery guidance.

**Verification**:
- Command/check: `rg -n "PySide6 is not installed|display backend|malformed event|subprocess failure|timeout state|AppImage or packaging review fails|Clean-room audit fails|CLI missing|non-zero|stderr" docs/troubleshooting.md`
...
- Command/check: `LC_ALL=C grep -RInP '[^\\x00-\\x7F]' docs/troubleshooting.md || true`
...
**Files Changed**:
- `README.md` - added source visual mode usage, settings flags, release status, and productization guide link.
- `docs/operator-runbook.md` - added source visual mode settings, failure state guidance, and productization guide link.
- `docs/visual-wrapper-boundary.md` - added Session 07 productization boundary and release gates.

**Verification**:
- Command/check: `rg -n "Visual Wrapper Source Mode|visual-wrapper-productization|source/dev|source visual mode|Session 07|plain fallback|CLI missing|Timed out|SHA256|release verification" README.md docs/operator-runbook.md docs/visual-wrapper-boundary.md`
...
- Command/check: `LC_ALL=C grep -RInP '[^\\x00-\\x7F]' README.md docs/operator-runbook.md docs/visual-wrapper-boundary.md || true`
  - Result: PASS - no non-ASCII output during the focused doc scan.
  - Evidence: prior doc scan produced no non-ASCII matches for edited docs.
- UI product-surface check: N/A - documentation only.
- UI craft check: N/A - documentation only.
...
**Files Changed**:
- `docs/visual-wrapper-productization.md` - added source visual mode productization guide.

**Verification**:
- Command/check: `rg -n "Source Setup|Run Commands|Smoke And Verification|Packaging Path|License And Release Gates|Clean-Room Verification|Session 08 Handoff" docs/visual-wrapper-productization.md`
  - Result: PASS - guide contains all required productization sections.
  - Evidence: section headings are present in the new document.
- Command/check: `LC_ALL=C grep -RInP '[^\\x00-\\x7F]' docs/visual-wrapper-productization.md || true`
...
**Files Changed**:
- `requirements-wrapper.txt` - clarified optional runtime and packaging review boundaries.
- `docs/visual-wrapper-productization.md` - documented optional dependency and packaging boundaries.

**Verification**:
- Command/check: `rg -n "PySide6|pyside6-deploy|Nuitka|AppImage|SHA256|source/relink" requirements-wrapper.txt docs/visual-wrapper-productization.md`
...
- Command/check: `LC_ALL=C grep -RInP '[^\\x00-\\x7F]' docs/visual-wrapper-productization.md requirements-wrapper.txt || true`
  - Result: PASS - no non-ASCII output.
  - Evidence: command produced no grep matches.
...
**Files Changed**:
- `src/apex_infinite_visual/events.py` - maps product failure states and details.
- `src/apex_infinite_visual/main.py` - maps launch/timeout/stderr/return-code failures and catches Qt startup failures.
- `src/apex_infinite_visual/qml/Main.qml` - renders failure, empty, offline, and waiting states.

**Verification**:
- Command/check: `QT_QPA_PLATFORM=offscreen python -m apex_infinite_visual.main --dry-run --max-iterations 1 --auto-close-ms 300 --effect-intensity 55 --font-scale 1.05 --plain-fallback`
  - Result: PASS - productized wrapper smoke exited 0 with no stderr.
  - Evidence: command completed successfully.
- Command/check: `python - <<'PY' ... import_qt_modules missing PySide6 assertion ... PY`
...
- Command/check: `rg -n "CLI missing|Display unavailable|Malformed event|Timed out|Subprocess stderr|Process failed|Plain fallback|Waiting for events|Offline|Visual wrapper unavailable" src/apex_infinite_visual/events.py src/apex_infinite_visual/main.py src/apex_infinite_visual/qml/Main.qml`
...
**Files Changed**:
- `src/apex_infinite_visual/settings.py` - defines clean-room preset tokens and plain fallback behavior.
- `src/apex_infinite_visual/main.py` - exposes theme names, effective theme, colors, and plain fallback to QML.
- `src/apex_infinite_visual/qml/Main.qml` - binds normal theme controls and colors to bridge properties.

**Verification**:
- Command/check: `python - <<'PY' ... preset assertions ... PY`
...
- Command/check: `rg -n "themeNames|effectiveThemeName|backgroundColor|setTheme|plainFallback" src/apex_infinite_visual/qml/Main.qml src/apex_infinite_visual/main.py src/apex_infinite_visual/settings.py`
...
- BQC Fixes: Contract alignment - Python settings, bridge properties, and QML controls use one preset contract (`src/apex_infinite_visual/settings.py`, `main.py`, `qml/Main.qml`).
...
**Files Changed**:
- `src/apex_infinite_visual/qml/Main.qml` - added productized controls, bridge-owned colors, font/scale usage, failure banner, and empty state.

**Verification**:
- Command/check: `pyside6-qmllint src/apex_infinite_visual/qml/Main.qml`
...
- Command/check: inspected `src/apex_infinite_visual/qml/Main.qml`
...
- BQC Fixes: Accessibility and product surface - new controls include `Accessible.name` and product-facing copy (`src/apex_infinite_visual/qml/Main.qml`).
...
**Files Changed**:
- `src/apex_infinite_visual/__init__.py` - exported productized wrapper settings and launcher error classes; updated package version label.
- `src/apex_infinite_visual/main.py` - added productized wrapper settings flags and argument validation.

**Verification**:
- Command/check: `python - <<'PY' ... import guard and help assertions ... PY`
...
- Command/check: `python -m py_compile src/apex_infinite_visual/__init__.py src/apex_infinite_visual/main.py`
...
- BQC Fixes: Contract alignment - exported settings and launch errors are stable public names without loading graphical dependencies (`src/apex_infinite_visual/__init__.py`).
...
**Files Changed**:
- `src/apex_infinite_visual/main.py` - added bridge settings properties and slots, process timeout option wiring, launch error mapping, and shutdown cleanup.

**Verification**:
- Command/check: `python - <<'PY' ... bridge assertions ... PY`
...
- Command/check: `python -m py_compile src/apex_infinite_visual/main.py`
  - Result: PASS - covered by the prior wrapper module compile command.
  - Evidence: command exited 0 during T004 verification.
...
- BQC Fixes:
  - Duplicate action prevention: `startRun()` ignores duplicate starts while in-flight (`src/apex_infinite_visual/main.py`).
  - State freshness on re-entry: fixture/process runs reset adapter snapshots before replay or launch (`src/apex_infinite_visual/main.py`).
  - Resource cleanup: `stopRun()` and `shutdown()` terminate subprocesses and stop timers (`src/apex_infinite_visual/main.py`).
...
- Validated the source-tree `src/apex_infinite/cli.py` script and project path before
...
**Files Changed**:
- `src/apex_infinite_visual/launcher.py` - added validation helpers, launch/timeout errors, timeout wait handling, and guarded command validation.

**Verification**:
- Command/check: `python - <<'PY' ... launcher validation assertions ... PY`
...
- Command/check: `python -m py_compile src/apex_infinite_visual/launcher.py`
  - Result: PASS - covered by the prior wrapper module compile command.
  - Evidence: command exited 0 during T004 verification.
...
- BQC Fixes: Resource cleanup and external dependency resilience - timeout waits terminate the subprocess and validation failures stop before launching (`src/apex_infinite_visual/launcher.py`).
...
**Files Changed**:
- `src/apex_infinite_visual/events.py` - added failure-state map and explicit failure detail handling.

**Verification**:
- Command/check: `python - <<'PY' ... EventStateAdapter failure mapping assertions ... PY`
...
- Command/check: `python -m py_compile src/apex_infinite_visual/events.py`
  - Result: PASS - covered by the prior wrapper module compile command.
  - Evidence: command exited 0 during T004 verification.
...
- BQC Fixes: Failure path completeness - explicit caller-visible failure states replace generic adapter errors (`src/apex_infinite_visual/events.py`).
...
**Files Changed**:
- `src/apex_infinite_visual/settings.py` - added validated presets, settings dataclass, bounds, and color export helpers.

**Verification**:
- Command/check: `python -m py_compile src/apex_infinite_visual/settings.py src/apex_infinite_visual/events.py src/apex_infinite_visual/launcher.py src/apex_infinite_visual/main.py src/apex_infinite_visual/__init__.py`
  - Result: PASS - wrapper modules compiled successfully.
  - Evidence: command exited 0.
- Command/check: `python - <<'PY' ... build_settings(theme_name='crt-amber', effect_intensity=70, font_family='monospace', font_scale=1.1) ... PY`
...
- BQC Fixes: Contract alignment - moved QML-facing theme/effect/font settings into a validated Python contract (`src/apex_infinite_visual/settings.py`).
...
**Verification**:
- Command/check: inspected `docs/visual-wrapper-boundary.md`
...
- Command/check: inspected `requirements.txt` and `requirements-wrapper.txt`
...
**Verification**:
- Command/check: `find src/apex_infinite_visual -maxdepth 3 -type f -print | sort`
...
- Command/check: inspected `src/apex_infinite_visual/events.py`, `launcher.py`, `main.py`, and `qml/Main.qml`
...
- Command/check: inspected `src/apex_infinite/events.py` and `docs/event-stream.md`
...
- Command/check: inspected `docs/visual-wrapper-spike.md` and `requirements-wrapper.txt`
