...
**Files Changed**:
- `src/apex_infinite/cli.py` - added scoped pylint disables for
  existing high-complexity functions touched by renderer wiring.
- `tests/conftest.py` - Black formatting only.
- `tests/test_prompts.py` - Black formatting only.
...
**Files Changed**:
- `tests/test_renderer.py` - added renderer width,
...
- Command/check: `python -m black --check tests/test_renderer.py`
...
- Command/check: `LC_ALL=C rg -n "[^\\x00-\\x7F]" tests/test_renderer.py || true`
...
**Files Changed**:
- `tests/test_cli_options.py` - added focused Click option
...
- Command/check: `python -m black --check tests/test_cli_options.py`
  - Result: PASS - test file is Black-formatted after running Black once.
  - Evidence: Black reported the file would be left unchanged.
- Command/check: `LC_ALL=C rg -n "[^\\x00-\\x7F]" tests/test_cli_options.py || true`
  - Result: PASS - no non-ASCII test characters were found.
  - Evidence: command produced no matches.
...
**Files Changed**:
- `tests/test_ui_config.py` - added focused resolver and
  validation tests.
...
**Verification**:
- Command/check: `python -m pytest tests/test_ui_config.py -q`
  - Result: PASS - resolver behavior and invalid config coverage passed.
  - Evidence: 16 tests passed.
- Command/check: `python -m black --check tests/test_ui_config.py`
  - Result: PASS - test file is Black-formatted.
  - Evidence: Black reported the file would be left unchanged.
- Command/check: `LC_ALL=C rg -n "[^\\x00-\\x7F]" tests/test_ui_config.py || true`
  - Result: PASS - no non-ASCII test characters were found.
  - Evidence: command produced no matches.
...
**Files Changed**:
- `README.md` - documented UI settings,
...
**Verification**:
- Command/check: `LC_ALL=C rg -n "Display Settings|--theme|Rendered labels|CLI flags -> environment constraints" README.md`
...
**Files Changed**:
- `src/apex_infinite/cli.py` - routed retry/fallback/interrupt/help/completion/safety states through `ApexRenderer`.
- `src/apex_infinite/ui.py` - updated retry renderer to accept the configured wait seconds.
...
**Verification**:
- Command/check: `python - <<'PY' ... fresh DB loop state smoke ... PY`
...
- Command/check: `python -m black --check src/apex_infinite/cli.py src/apex_infinite/ui.py`
  - Result: PASS - modified modules are Black-formatted.
  - Evidence: Black reported both files would be left unchanged.
- Command/check: `python -m pytest tests/test_prompts.py -q`
  - Result: PASS - existing prompt/routing tests still pass.
  - Evidence: 62 tests passed.
- Command/check: `LC_ALL=C rg -n "[^\\x00-\\x7F]" src/apex_infinite/cli.py src/apex_infinite/ui.py || true`
...
**BQC Fixes**:
- Failure path completeness: LLM retry/final failure, JSON fallback, help, interrupt, completion, and safety-stop paths now have explicit renderer states (`src/apex_infinite/cli.py`).
- Contract alignment: SIGINT uses the active renderer when available while preserving legacy behavior before renderer initialization (`src/apex_infinite/cli.py`).
...
**Files Changed**:
- `src/apex_infinite/cli.py` - routed Codex execution states through `ApexRenderer` and `CodexCommandSnapshot`.
...
**Verification**:
- Command/check: `python - <<'PY' ... execute_codex(...) branch checks ... PY`
...
**BQC Fixes**:
- Failure path completeness: subprocess timeout, missing binary, non-zero exit, and generic exception paths now render explicit severity states (`src/apex_infinite/cli.py`).
- Contract alignment: renderer display is separate from the raw `execute_codex()` return value consumed by manager/history logic (`src/apex_infinite/cli.py`).
...
**Files Changed**:
- `src/apex_infinite/cli.py` - routed loop status and prompt preview surfaces through `ApexRenderer`.
...
**Verification**:
- Command/check: `HOME=/tmp/apex-t012-home apex-infinite --path . --start implement --theme crt-green --dry-run --max-iterations 1 | sed -n '1,180p'`
...
- Command/check: `HOME=/tmp/apex-t012-home-plain apex-infinite --path . --start implement --plain --ascii --compact --dry-run --max-iterations 1 | sed -n '1,180p'`
...
**BQC Fixes**:
- Product surface discipline: iteration and manager state now use product/operator labels instead of raw ad hoc console lines (`src/apex_infinite/cli.py`).
- External dependency resilience: verification avoided network manager calls by using a zero-history start-command path; the existing retry path remains unchanged for later routing (`src/apex_infinite/cli.py`).
...
**Files Changed**:
- `src/apex_infinite/cli.py` - routed history display through `ApexRenderer.print_history()`.
...
**Verification**:
- Command/check: `python - <<'PY' ... temp DB db_show_history(renderer=...) ... PY`
...
**BQC Fixes**:
- Contract alignment: history renderer consumes row dictionaries and does not alter raw SQLite values (`src/apex_infinite/cli.py`, `src/apex_infinite/ui.py`).
- State freshness on re-entry: each history render fetches current rows from SQLite before display (`src/apex_infinite/cli.py`).
...
**Files Changed**:
- `src/apex_infinite/cli.py` - routed startup and interactive prompt surfaces through `ApexRenderer` and `StartupSnapshot`.
...
**Verification**:
- Command/check: `apex-infinite --path . --theme crt-amber --max-iterations 0 --dry-run --ceo secret-value | sed -n '1,100p'`
...
- Command/check: `apex-infinite --path . --plain --ascii --compact --max-iterations 0 --dry-run --ceo secret-value | sed -n '1,100p'`
...
**BQC Fixes**:
- Error information boundaries: startup no longer echoes full CEO instruction text in the normal product surface (`src/apex_infinite/cli.py`).
- Product surface discipline: interactive startup uses semantic product labels instead of raw debug/status copy (`src/apex_infinite/cli.py`).
...
**Notes**:
- Imported UI resolver and renderer helpers into `src/apex_infinite/cli.py`.
...
**Files Changed**:
- `src/apex_infinite/cli.py` - wired UI resolver, renderer instantiation, invalid UI config handling, and optional renderer parameters.
...
**Verification**:
- Command/check: `apex-infinite --path . --theme missing --max-iterations 0 --dry-run; printf 'exit=%s\n' $?`
...
- Command/check: `apex-infinite --path . --theme crt-amber --max-iterations 0 --dry-run | sed -n '1,80p'`
...
**BQC Fixes**:
- Contract alignment: optional renderer parameters preserve existing helper call contracts while allowing display routing (`src/apex_infinite/cli.py`).
- Failure path completeness: invalid UI configuration exits before loop execution with a clear operator error (`src/apex_infinite/cli.py`).
...
**Files Changed**:
- `src/apex_infinite/cli.py` - added the four UI Click flags and parameters.
...
**BQC Fixes**:
- Contract alignment: Click now exposes the CLI controls required by the UI resolver (`src/apex_infinite/cli.py`, `src/apex_infinite/ui.py`).
...
**Files Changed**:
- `src/apex_infinite/config.yaml` - added default UI configuration keys and built-in values.
...
**Verification**:
- Command/check: `python - <<'PY' ... yaml.safe_load(...); resolve_ui_settings(...) ... PY`
...
- Command/check: `LC_ALL=C rg -n "[^\\x00-\\x7F]" src/apex_infinite/config.yaml || true`
  - Result: PASS - no non-ASCII source characters were found.
  - Evidence: command produced no matches.
- Command/check: `file src/apex_infinite/config.yaml`
  - Result: PASS - file remains ASCII text.
  - Evidence: reported `src/apex_infinite/config.yaml: ASCII text`.
...
**BQC Fixes**:
- Contract alignment: default config keys now match the resolver-supported UI schema (`src/apex_infinite/config.yaml`, `src/apex_infinite/ui.py`).
...
**Files Changed**:
- `src/apex_infinite/ui.py` - added renderer snapshots, semantic renderer helpers, plain/ASCII rendering, and final resolver cleanup.
...
**Verification**:
- Command/check: `python - <<'PY' ... ApexRenderer(...) ... PY`
...
- Command/check: `python - <<'PY' ... plain_renderer.print_error(...) ... PY`
...
- Command/check: `python -m black --check src/apex_infinite/ui.py`
  - Result: PASS - helper module is Black-formatted.
  - Evidence: Black reported the file would be left unchanged.
- Command/check: `python -m pylint src/apex_infinite/ui.py`
  - Result: PASS - helper module passes lint.
  - Evidence: pylint rated the module 10.00/10.
- Command/check: `LC_ALL=C rg -n "[^\\x00-\\x7F]" src/apex_infinite/ui.py || true`
  - Result: PASS - no non-ASCII source characters were found.
  - Evidence: command produced no matches.
...
**BQC Fixes**:
- Contract alignment: renderer accepts typed snapshots/settings and keeps rendering separate from workflow data (`src/apex_infinite/ui.py`).
- Failure path completeness: errors, warnings, timeouts, missing commands, parse fallback, interrupts, and safety stops have explicit severity-capable render helpers (`src/apex_infinite/ui.py`).
- Product surface discipline: renderer labels are product/operator-facing and avoid implementation telemetry on the normal surface (`src/apex_infinite/ui.py`).
...
**Files Changed**:
- `src/apex_infinite/ui.py` - added settings resolver, environment constraint detection, UI config readers, and malformed-config errors.
...
**Verification**:
- Command/check: `python - <<'PY' ... resolve_ui_settings(...) ... PY`
...
**BQC Fixes**:
- Trust boundary enforcement: `ui` config shape, boolean values, effect level, and theme names are validated before use (`src/apex_infinite/ui.py`).
- Failure path completeness: invalid resolver inputs raise field-specific `UiConfigError` instead of silently falling back (`src/apex_infinite/ui.py`).
- Contract alignment: CLI/config/environment precedence is centralized in one resolver used by later wiring (`src/apex_infinite/ui.py`).
...
**Files Changed**:
- `src/apex_infinite/ui.py` - added custom theme schema validation and registry building.
...
**Verification**:
- Command/check: `python - <<'PY' ... build_theme_registry(...) ... PY`
...
**BQC Fixes**:
- Trust boundary enforcement: `ui.themes` user config is schema-validated before use (`src/apex_infinite/ui.py`).
- Failure path completeness: malformed theme config now has explicit caller-visible `UiConfigError` messages (`src/apex_infinite/ui.py`).
...
**Notes**:
- Created `src/apex_infinite/ui.py` with immutable dataclasses for semantic theme
...
**Files Changed**:
- `src/apex_infinite/ui.py` - added UI token, glyph, override, and settings models plus built-in definitions.
...
**Verification**:
- Command/check: `python -c "from apex_infinite_ui import BUILT_IN_THEME_NAMES, BUILT_IN_THEMES, ASCII_GLYPHS, UNICODE_GLYPHS; print(BUILT_IN_THEME_NAMES); print(sorted(BUILT_IN_THEMES)); print(ASCII_GLYPHS.name, UNICODE_GLYPHS.name)"`
...
**BQC Fixes**:
- Contract alignment: created semantic dataclasses so later renderer and config code use an explicit shape instead of ad hoc dictionaries (`src/apex_infinite/ui.py`).
...
**Notes**:
- Mapped operator output in `src/apex_infinite/cli.py`: signal interrupt/force quit,
...
- Command/check: `sed -n '620,1040p' src/apex_infinite/cli.py` and `sed -n '1040,1130p' src/apex_infinite/cli.py`
...
- Command/check: `python -c "import sys; print(sys.executable); import click, rich, yaml, openai; print('runtime ok')"`
  - Result: PASS - CLI-local virtualenv imports runtime dependencies.
  - Evidence: printed the active local Python executable and `runtime ok`.
- Command/check: `python -m pytest tests/test_prompts.py -v`
  - Result: PASS - prompt/routing baseline passes.
  - Evidence: 62 tests passed in 10.39s.
- Command/check: `python -m black --version && python -m pylint --version | sed -n '1,5p'`
