...
- `docs/visual-wrapper-boundary.md` - untracked
- `docs/transcripts/README_transcripts.md` - untracked
- `docs/transcripts/dry-run-plain.txt` - untracked
- `docs/transcripts/history-ledger.txt` - untracked
- `docs/transcripts/machine-output-events.jsonl` - untracked
...
- Script smoke: `bash .spec_system/scripts/analyze-project.sh --json | jq .` - PASS - current session resolves to `phase00-session05-docs-samples-and-runbooks`.
- Script smoke: `bash .spec_system/scripts/check-prereqs.sh --json --env | jq .` - PASS - overall status `pass`.
- JSONL sample: `jq -c . docs/transcripts/machine-output-events.jsonl` - PASS - all 19 lines parse as JSON.
