#!/usr/bin/env bash
# Manage project-local Ollama Docker service and the configured model.
...
Actions:
  up        Start Ollama and ensure OLLAMA_MODEL is present (default)
  ...
  restart   Restart Ollama and ensure OLLAMA_MODEL is present
...
Environment:
  ...
  OLLAMA_HOST, OLLAMA_PORT, OLLAMA_MODEL, OLLAMA_DOCKER_*,
  OLLAMA_KEEP_ALIVE, OLLAMA_CHAT_CHECK_TIMEOUT.
EOF
}
...
OLLAMA_DOCKER_PROGRESS="${OLLAMA_DOCKER_PROGRESS:-plain}"
OLLAMA_CHAT_CHECK_TIMEOUT="${OLLAMA_CHAT_CHECK_TIMEOUT:-90}"

export OLLAMA_HOST OLLAMA_PORT OLLAMA_MODEL
export OLLAMA_DOCKER_CONTAINER OLLAMA_DOCKER_WAIT_SECONDS OLLAMA_DOCKER_GPU
export OLLAMA_DOCKER_PROGRESS OLLAMA_CHAT_CHECK_TIMEOUT
...
compose_args() {
    printf '%s\0' compose --ansi never --progress "$OLLAMA_DOCKER_PROGRESS" -f "${CLI_DIR}/docker-compose.ollama.yml"
...
model_is_present() {
    docker_compose exec -T ollama ollama list \
        | awk -v model="$OLLAMA_MODEL" 'NR > 1 && $1 == model { found = 1 } END { exit !found }'
}
...
if [[ "$CHAT_CHECK" == "1" && "$ACTION" != "down" && "$ACTION" != "logs" ]]; then
    "${SCRIPT_DIR}/check-ollama.sh" --chat --timeout "$OLLAMA_CHAT_CHECK_TIMEOUT"
fi
